...
namespace SpaceWhale.HierarchyInspector.Editor
{
    ...
    internal static partial class HierarchyOverlay
    {
        ...
        private static int ComputeHoveredIconIndex(in HierarchyItemInfo info, Rect selectionRect, Vector2 mousePos)
        {
            ...
            float iconStartX = GetIconStripRightX(selectionRect, info.isPrefabInstance);
        ...
        private const float PrefabArrowWidth = 16f;
        private const float PrefabArrowGap = 2f;
        ...
        // Right edge of the icon strip; left side of the active-toggle reserve. Prefab instance
        // rows reserve an extra slot for the "open prefab" arrow drawn between the icons and the toggle.
        private static float GetIconStripRightX(in Rect selectionRect, bool isPrefabInstance = false)
        {
            float toggleReserve = T.ActiveToggle ? ToggleWidth + ToggleReservePadding : ToggleReservePadding;
            float prefabReserve = isPrefabInstance ? PrefabArrowWidth + PrefabArrowGap : 0f;
            return selectionRect.xMax - toggleReserve - prefabReserve;
        }
        ...
        /// <summary>Raw prefab-instance flag (consumers gate on T.PrefabTinting / T.PrefabOverrideDot themselves) and override-dot detection.</summary>
        private static void RefreshPrefabStatus(ref HierarchyItemInfo info, GameObject go)
        {
            info.isPrefabInstance = PrefabUtility.IsPartOfPrefabInstance(go);
        ...
        private static void HandleMouseMovePass(EntityId entityId, Rect selectionRect)
        {
            ...
            float hoverIconStripStart = iconCount > 0
                ? GetIconStripRightX(selectionRect, hasInfo && hoverInfo.isPrefabInstance) - iconCount * (hoverIconSize + IconGap)
                : hoverViewW;
            ...
            EventType eventType, bool cacheHit, HierarchyItemInfo cached)
        {
            ...
            DrawActiveToggle(go, selectionRect);
            DrawPrefabOpenArrow(go, in info, selectionRect, isRepaint);
