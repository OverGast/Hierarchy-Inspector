...
namespace SpaceWhale.HierarchyInspector.Editor
{
    ...
    {
        private static Texture _prefabOpenArrowIcon;

        private static void DrawPrefabOpenArrow(GameObject go, in HierarchyItemInfo info, Rect selectionRect, bool isRepaint)
        {
            if (!info.isPrefabInstance) return;

            if (_prefabOpenArrowIcon == null)
            {
                var content = EditorGUIUtility.IconContent("ArrowNavigationRight")
                    ?? EditorGUIUtility.IconContent("tab_next@2x");
                _prefabOpenArrowIcon = content != null ? content.image : null;
            }

            float toggleReserve = T.ActiveToggle ? ToggleWidth + ToggleReservePadding : ToggleReservePadding;
            float arrowX = selectionRect.xMax - toggleReserve - PrefabArrowWidth;
            float arrowY = selectionRect.y + (selectionRect.height - PrefabArrowWidth) / 2f;
            var arrowRect = new Rect(arrowX, arrowY, PrefabArrowWidth, PrefabArrowWidth);

            if (isRepaint && _prefabOpenArrowIcon != null)
            {
                bool isHovered = arrowRect.Contains(Event.current.mousePosition);
                var prevColor = GUI.color;
                GUI.color = new Color(1f, 1f, 1f, isHovered ? 1f : 0.65f);
                GUI.DrawTexture(arrowRect, _prefabOpenArrowIcon, ScaleMode.ScaleToFit);
                GUI.color = prevColor;
            }
