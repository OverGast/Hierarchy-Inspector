...
namespace FoundFootage.Editor.Hierarchy
{
    ...
    {
        // Pro-skin only.
        private static readonly Color _editorBgPro = new Color(0.22f, 0.22f, 0.22f, 1f);
        private static Color EditorBg => _editorBgPro;
            ...
            bool isSelected)
        {
            ...
            // StripeAlpha treated as a Lerp factor so changing either field takes visible effect.
            Color stripeAdjustedBase = isStripeRow
                ? Color.Lerp(EditorBg, T.StripeBaseColor, T.StripeAlpha)
                : EditorBg;
            ...
            // DrawNativeFoldoutArrow re-paints the arrow afterwards on top of any stripe behind it.
            if (T.LeftColorStripe && hasCustomColor)
            {
                float stripeX = Mathf.Max(GearColumnRightX, selectionRect.x - 8f);
                QueueRect(new Rect(stripeX, fullRowRect.y, 3f, fullRowRect.height), data.BackgroundColor);
            }
            ...
            bool isSelected)
        {
            ...
            float iconStripLeftX = iconCount > 0
                ? GetIconStripRightX(selectionRect) - iconCount * (T.IconSize + IconGap)
                : selectionRect.xMax;
