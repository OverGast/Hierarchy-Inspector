...
1. Download the latest `HierarchyDecorator.unitypackage` from the releases page.
...
> 📷 **[SCREENSHOT: A standard Unity hierarchy with Hierarchy Decorator enabled, showing alternating rows, a gear button visible on the hovered row, and component icons in the gutter.]**

If none of this is happening, check that the active theme is selected: open **Edit → Preferences → Hierarchy Decorator** and pick a theme asset. A default one ships with the package.

{% hint style="info" %}
**The first time you click the gear** on a GameObject, Hierarchy Decorator adds a small `HierarchyInspectorData` component to it. This component is invisible in the Inspector by default and is only present on objects you have actually styled. Untouched objects stay clean.
{% endhint %}
...
| --- | --- |
| Switch themes, create a new theme, reset to defaults | **Edit → Preferences → Hierarchy Decorator** |
