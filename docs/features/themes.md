...
![Unity Preferences with the Hierarchy Inspector pane open](../../.gitbook/assets/02-preferences-pane.png)
...
Each tab is divided into named sections (foldouts). Settings inside a section are usually a parent toggle followed by detail fields. When you turn off a parent toggle, the detail fields gray out and indent so you can see they exist but are not currently in effect. This is **show\_if** behavior; the detail fields stay visible for discovery, they just become read-only until you re-enable the parent.
...
{% hint style="info" %}
**Theme assets travel with the project.** Anyone who pulls the project gets the same themes. The user's _active_ selection is the only thing that's per-user.
{% endhint %}
